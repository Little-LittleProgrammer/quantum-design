...
import { js_tools_get_table_header_columns } from '@/assets/ts/tools';
...
export default defineComponent({
    ...
    setup() {
        ...
        const unRefData = {
            ...
            tableHeader: js_tools_get_table_header_columns({auth_name: '菜单名称', path: '页面地址', path_type: '类型', icon: '图标', action: '操作' }, {
                alignData: {
                    all: 'left',
                    path_type: 'center',
                    icon: 'center'
        ...
        const scroll_to_input = (data: menuData) => {
            setTimeout(() => {
        ...
        // 删除菜单
        const delete_menu = (obj: menuData) => {
        ...
        const create_message = (obj:menuData, type: 'add' | 'edit' | 'del') => {
            const _typeObj = {
                'add': '添加',
                'edit': '修改',
                'del': '删除'
        ...
        // 取消添加、编辑菜单或接口
        const cancel_menu = (obj: menuData) => {
            obj.edit = 0;
        ...
        const front_remove_menu = (data: menuData, arr: menuData[], parentData?: menuData) => {
            arr.some((_n, i) => {
                if (arr[i].id === data.id) {
                    arr.splice(i, 1);
        ...
        // 递归获取指定id节点下的所有一级子节点
        const get_node_by_id = (id: number, arr: menuData[]) => {
            for (let i = 0; i < arr.length; i++) {
                if (arr[i].id == id) {
                    data.sonIds = arr[i].children!.map((item: menuData) => item.id!);
                    break;
                } else if (arr[i].children && arr[i].children!.length > 0) {
