...
import {QDrawer, useDrawerInner} from '@wuefront/vue3-antd-ui';
...
interface DataProps {
    ...
    authTreeData: menuData[],
    pageTreeData: menuData[],
    pagePidArr: number[],
    leafIdArr: never[]
}
interface IAuthListEx extends menuData {
    scopedSlots?: {
        title: string
    }
}
...
const get_tree_data = async() => {
    ...
    data.authTreeData = deep_copy(_treeData.data.list); // 权限树结构
    data.pageTreeData = deep_copy(_treeData.data.list); // 页面树结构
...
// 递归 去掉接口
const filter_tree_remove_api = (arr:menuData[]) => {
    const _arr = arr;
    _arr.forEach(item => {
...
// 递归 获取页面的pid数据
const get_pid_arr = (list: menuData[]) => {
    list.forEach(item => {
