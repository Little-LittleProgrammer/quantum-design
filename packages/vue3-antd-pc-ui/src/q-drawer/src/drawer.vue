...
<template>
    <a-drawer class="q-drawer drawer-container add-weight" v-bind="getBindValues" @close="on_close">
        ...
        <drawer-footer v-bind="getProps" @close="on_close" @ok="handle_ok" :height="getFooterHeight">
...
const emit = defineEmits(['register', 'visibleChange', 'ok', 'close']);
...
const getMergeProps = computed((): DrawerProps => {
    return js_utils_deep_merge(toRaw(props), unref(propsRef));
});
...
function set_drawer_props(props: Partial<DrawerProps>): void {
    ...
    propsRef.value = js_utils_deep_merge(unref(propsRef) || ({} as any), props);
