			...
			>
				...
				<template v-if="getSummaryData.length > 0" #summary>
					<table-summary v-bind="getBindValues.summaryConfig">
						<table-summary-row
							v-for="(item, _index) in getSummaryData"
							:key="_index"
						>
							<table-summary-cell
								v-if="!!getRowSelectionRef"
							></table-summary-cell>
							<table-summary-cell
								class="table-summary-cell"
								v-for="(e, _index2) in getFlatColumns"
								:key="_index2"
								:index=" !!getRowSelectionRef ? _index2 + 1 : _index2 "
							>
