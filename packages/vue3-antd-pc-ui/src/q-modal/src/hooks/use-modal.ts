import type { ExtendedModalApi, ModalApiOptions, ModalProps } from '../types/modal';
...
import QAntdModal from '../modal.vue';

const USER_MODAL_INJECT_KEY = Symbol('QAntd_MODAL_INJECT');
...
export function useQAntdModal<TParentModalProps extends ModalProps = ModalProps>(
...
async function checkProps(api: ExtendedModalApi, attrs: Record<string, any>) {
    ...
    for (const attr of Object.keys(attrs)) {
        if (stateKeys.has(attr) && !['class'].includes(attr)) {
            ...
            console.warn(
                `[QAntd Modal]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Modal, please use useQAntdModal or api.`
            );
        }
    }
}
