import { date_util, isNumber, isObject } from '@wuefront/utils';
...
export function process_date_value(attr: Record<string, any>, component: string) {
    ...
    if (valueFormat) {
        attr.value = isObject(value) ? date_util(value as any).format(valueFormat) : value;
    } else if (DATE_TYPE.includes(component) && value) {
        attr.value = date_util(attr.value);
    }
}
