    ...
    setup(props, {slots}) {
        ...
        function handle_rules():Rule[] {
            ...
            if (_characterInx !== -1 && !_rules[_characterInx].validator) {
                _rules[_characterInx].message = _rules[_characterInx].message || `字符因小于${[_rules[_characterInx].max] as Record<string, any>}位`;
            }
            return _rules;
        }
        function render_component() {
            ...
            const _eventKey = `on${first_to_upper(changeEvent)}`;
            const _on = {
            ...
            return (
                <span>
                    <a-tooltip
                        placement="top"
                        class="mr"
                        title={<div style={getTooltipStyle}>{helpMessage}</div>}
