...
<template>
    <div class="qm-tree-table" style="margin: 10px">
        <!-- 表格头部 -->
        <tree-table-cell class="qm-tree-table-cell-border-b qm-tree-table-header" :data="data.headerTree" :header="data.headerFormat">
        ...
        <!-- 列表数据 -->
        <tree-table-cell v-for="(item, index) in data.dataTree" :class="{ 'qm-tree-table-cell-border-b': index !== props.treeData.length - 1 }" :key="item.key" :rootIndex="index" :data="item" :header="data.headerFormat">
            <template #default="scope">
