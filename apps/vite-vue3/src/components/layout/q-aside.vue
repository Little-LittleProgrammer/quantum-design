...
const refAsideMenu = ref<Nullable<HTMLElement>>(null);
...
watch(() => props.menuData, (val) => {
    ...
    setTimeout(() => {
        ...
        if (val.length == 0) {
            ...
            setTimeout(() => {
                // 选中导航超出自动滚动到中间位置
                const _height = (refAsideMenu.value as any).$el.offsetHeight;
                const $selectedMenu: NodeListOf<HTMLElement> = (refAsideMenu.value as any).$el.querySelectorAll(
                    '.ant-menu-item-selected'
                );
                if ($selectedMenu.length > 0) {
                    const _topNum = $selectedMenu[0].offsetTop;
                    const _scrollTopNum = (refAsideMenu.value as any).$el.scrollTop;
                    if (_topNum >= _height + _scrollTopNum - 60) {
                        (refAsideMenu.value as unknown as HTMLElement).scrollTo(0, _topNum - 200);
                    }
                }
            }, 500);
        }
