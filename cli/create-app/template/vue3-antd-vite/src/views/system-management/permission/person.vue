        ...
        <a-card class="g-card mt" :size="antdStore.cardSize">
            ...
            >
                <template #bodyCell="{column, record, text}">
                    <template v-if="column.dataIndex === 'role_id_arr'">
                        <a-tag v-for="item in text" :key="item">{{ data.roleOptions.find(e => (e.value as number) === item)?.label }}</a-tag>
                    </template>
...
const getEditSchemas = computed(():FormSchema[] => {
    return [{
        ...
        required: true,
        componentProps: {
            disabled: data.addEditUserDataPop.type === 'edit'
        }
    }, {
        label: '企业邮箱',
        field: 'email',
