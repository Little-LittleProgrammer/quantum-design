            ...
            for (const dir of _dirsList) {
                ...
                if (pkgContent) {
                    ...
                    if (_text.devDependencies){
                        ...
                        for (const key in _text.devDependencies) {
                            _text.devDependencies[key] = 'catalog:';
                        }
                    }
                    if (_text.dependencies) {
                        for (const key in _text.dependencies) {
                            _text.dependencies[key] = 'catalog:';
                        }
                    }
                    writeFileSync(`${dir}/package.json`, JSON.stringify(_text, null, 4));
                }
